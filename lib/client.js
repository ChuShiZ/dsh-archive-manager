	...
	function apply(ctx) {
		...
		function ArchivedList(props) {
			...
			React.useEffect(() => {
				...
				let off1, off2;
				try {
					off1 = ctx.on?.("locale/change", update);
				} catch {}
				try {
					off2 = ctx.locale?.subscribe?.(update);
				} catch {}
				return () => {
					try {
						off1?.();
			...
			const renderRow = (s) => {
				...
				}, titleOf(s) + (previewOpen ? "  ▲" : "  ▾")), React.createElement("div", { className: "ds-ardl-sub" }, subOf(s)), snippetList.length > 0 ? React.createElement("div", { className: "ds-ardl-snippets" }, snippetList.map(renderHit), typeof s.matchCount === "number" && s.matchCount > snippetList.length ? React.createElement("div", { className: "ds-ardl-snippet-more" }, t.moreHits(s.matchCount - snippetList.length)) : null) : null, previewOpen ? React.createElement("div", { className: "ds-ardl-preview" }, previewMsgs === void 0 ? React.createElement("div", { className: "ds-ardl-preview-loading" }, t.previewLoading) : previewMsgs.length === 0 ? React.createElement("div", { className: "ds-ardl-preview-empty" }, t.previewEmpty) : previewMsgs.map((m, idx) => React.createElement("div", {
					key: idx,
					className: "ds-ardl-preview-msg " + m.role
				}, React.createElement("span", { className: "ds-ardl-preview-role" }, m.role === "user" ? t.roleUser : m.role === "assistant" ? t.roleAssistant : m.role + "："), React.createElement("span", { className: "ds-ardl-preview-text" }, m.text)))) : null), confirming ? React.createElement("div", { className: "ds-ardl-actions" }, React.createElement("button", {
					className: "ds-ardl-btn primary",
					disabled: busyId === s.id,
					onClick: () => doDelete(s.id)
				}, busyId === s.id ? t.deleting : t.confirmDelete), React.createElement("button", {
					className: "ds-ardl-btn",
					disabled: busyId === s.id,
					onClick: () => setConfirmId(void 0)
				}, t.cancel)) : React.createElement("div", { className: "ds-ardl-actions" }, React.createElement("button", {
					className: "ds-ardl-btn restore",
					disabled: busyId === s.id,
					onClick: () => doRestore(s.id)
				}, t.restoreOpen), React.createElement("button", {
					className: "ds-ardl-btn danger",
					disabled: busyId === s.id,
					onClick: () => setConfirmId(s.id)
				}, t.delete)));
			};
			const body = isBusy && rows.length === 0 ? React.createElement("div", { className: "ds-ardl-empty" }, searching ? t.searching : t.loading) : rows.length === 0 ? React.createElement("div", { className: "ds-ardl-empty" }, query.trim() ? t.emptyNoMatch : t.emptyNoArchived) : groups.map(([ws, items]) => React.createElement("div", {
				key: ws,
			...
			}, c.label))), selectedIds.size > 0 ? React.createElement("div", { className: "ds-ardl-batch-bar" }, React.createElement("span", { className: "ds-ardl-batch-info" }, t.batchSelected(selectedIds.size)), batchConfirm ? React.createElement(React.Fragment, null, React.createElement("button", {
				className: "ds-ardl-btn primary",
				disabled: batchBusy,
				onClick: doBatchDelete
			}, batchBusy ? t.deleting : t.confirmDeleteSelected), React.createElement("button", {
				className: "ds-ardl-btn",
				disabled: batchBusy,
			...
			}, t.deleteSelected)) : null, React.createElement("div", { className: "ds-ardl-hint" }, t.hint), !loading ? React.createElement("div", { className: "ds-ardl-count" }, countText + (searching ? t.searchingSuffix : "")) : null, error ? React.createElement("div", { className: "ds-ardl-err" }, error) : null, React.createElement("div", { className: "ds-ardl-list" }, body));
		}
		slots.inject("settings.section", () => {
			...
			let off1, off2;
			try {
				off1 = ctx.on?.("locale/change", updateLabel);
			} catch {}
			try {
				off2 = ctx.locale?.subscribe?.(updateLabel);
			} catch {}
			return () => {
				try {
					off1?.();
