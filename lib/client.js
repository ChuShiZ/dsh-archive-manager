	...
	*/
	...
	function apply(ctx) {
		...
		const getManager = () => {
			...
			const httpCall = (path, body) => fetch(path, body ? {
				method: "POST",
		...
		function ArchivedList(props) {
			...
			const renderRow = (s) => {
				...
				}, React.createElement("div", { className: "ds-ardl-meta" }, React.createElement("div", { className: "ds-ardl-title" }, titleOf(s)), React.createElement("div", { className: "ds-ardl-sub" }, subOf(s)), snippetList.length > 0 ? React.createElement("div", { className: "ds-ardl-snippets" }, snippetList.map((h, i) => React.createElement("div", {
					key: i,
					className: "ds-ardl-snippet",
					title: h.snippet
				}, h.snippet)), typeof s.matchCount === "number" && s.matchCount > snippetList.length ? React.createElement("div", { className: "ds-ardl-snippet-more" }, "… 还有 " + (s.matchCount - snippetList.length) + " 处命中") : null) : null), confirming ? React.createElement("div", { className: "ds-ardl-actions" }, React.createElement("button", {
			...
			}), React.createElement("div", { className: "ds-ardl-hint" }, "支持全文：检索会话内的用户消息、AI 回复与工具调用内容（FTS5），每个会话最多展示 5 条命中，空输入显示全部归档"), !loading ? React.createElement("div", { className: "ds-ardl-count" }, countText + (searching ? "  ·  搜索中…" : "")) : null, error ? React.createElement("div", { className: "ds-ardl-err" }, error) : null, React.createElement("div", { className: "ds-ardl-list" }, body));
		}
