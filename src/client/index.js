          ...
          previewOpen
            ? React.createElement('div', { className: 'ds-ardl-preview' },
                previewMsgs === undefined
                  ? React.createElement('div', { className: 'ds-ardl-preview-loading' }, '加载预览中…')
                  : (previewMsgs.length === 0
                      ? React.createElement('div', { className: 'ds-ardl-preview-empty' }, '无可预览消息')
                      : previewMsgs.map((m, idx) => React.createElement('div', { key: idx, className: 'ds-ardl-preview-msg ' + m.role },
                          React.createElement('span', { className: 'ds-ardl-preview-role' }, m.role === 'user' ? '用户：' : m.role === 'assistant' ? 'AI：' : m.role + '：'),
    ...
    return React.createElement(React.Fragment, null,
      ...
      selectedIds.size > 0
        ? React.createElement('div', { className: 'ds-ardl-batch-bar' },
            React.createElement('span', { className: 'ds-ardl-batch-info' }, '已选 ' + selectedIds.size + ' 个'),
            batchConfirm
              ? React.createElement(React.Fragment, null,
                  React.createElement('button', { className: 'ds-ardl-btn primary', disabled: batchBusy, onClick: doBatchDelete }, batchBusy ? '删除中…' : '确认删除所选'),
                  React.createElement('button', { className: 'ds-ardl-btn', disabled: batchBusy, onClick: () => setBatchConfirm(false) }, '取消')
                )
              : React.createElement('button', { className: 'ds-ardl-btn danger', disabled: batchBusy, onClick: () => setBatchConfirm(true) }, '删除所选')
          )
        : null,
